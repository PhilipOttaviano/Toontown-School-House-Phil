...
def pickSuitAttack(attacks, suitLevel):
    ...
    count = 0
    index = 0
    total = sum(attack[3][suitLevel] for attack in attacks)
    for attack in attacks:
        count += attack[3][suitLevel]
        if randNum < count:
            attackNum = index
...
SuitStatuses = [{'name': 'lured', 'rounds': 1, 'kbBonus': 0.5, 'decay': 100, 'toons': [], 'levels': []},
                {'name': 'soaked', 'rounds': 1, 'defense': -15, 'toons': [0]},
                {'name': 'overclocked', 'weakness': 0.50},
                {'name': 'trapped', 'level': -1, 'damage': 0, 'toon': 0},
                {'name': 'cogs-miss', 'rounds': 1}]
LURED_STATUS = 0
SOAKED_STATUS = 1
TRAPPED_STATUS = 2
COGS_MISS_STATUS = 3
...
SuitAttributes = {'f': {'name': TTLocalizer.SuitFlunky,  # cog name
                  ...
                  'ms': {'name': TTLocalizer.SuitMoverShaker,
                         ...
                         'attacks':
                             (('BrainStorm',
                               (5, 6, 8, 10, 12, 16, 21, 25),
                               (60, 75, 80, 85, 90, 90, 90, 90),
                               (15, 15, 15, 15, 15, 15, 15, 15)),
                              ('HalfWindsor',
                               (6, 9, 11, 13, 16, 20, 22, 24),
                               (50, 65, 70, 75, 80, 80, 80, 80),
                               (20, 20, 20, 20, 20, 20, 20, 20)),
                              ('Quake',
                               (9, 12, 15, 18, 21, 25, 28, 34),
                               (60, 65, 75, 80, 85, 85, 85, 85),
                               (20, 20, 20, 20, 20, 20, 20, 20)),
                              ('Shake',
                               (6, 8, 10, 12, 14, 18, 22, 27),
...
def getAttackTauntIndex(attackName):
    if attackName not in SuitAttackTaunts:
        return 1
